...
export default function Events() {
  ...
  const upload = (data) => {
    return new Promise((resolve, reject) => {
      ...
      const storageRefVar = storageRef(storage, `/img/Events/${data.title}`);
  ...
  function firebaseWrite(inputData) {
    return new Promise((resolve, reject) => {
      ...
      set(databaseRef(db, `admin/Events/${key}`), inputData)
        .then(() => {
          resolve();
        })
        .catch((error) => {
  ...
  function firebaseGet(key) {
    const dataRef = databaseRef(db, `admin/Events/${key}`);
    ...
    onValue(usersRef, (snapshot) => {
      ...
      for (let obj in dataArray) {
        if (dataArray[obj].title === inputData.title) {
          ...
          set(databaseRef(db, `admin/Events/${dataArray[obj].key}`), inputData);
          break;
        }
      }
  ...
  async function firebaseDelete(key) {
    const dbRef = databaseRef(db, 'admin/Events/' + key);
  ...
  function StorageDelete(data){
    const storage = getStorage(app);
    const desertRef = storageRef(storage, `/img/Events/${data.title}`);
