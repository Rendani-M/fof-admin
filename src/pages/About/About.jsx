...
export default function About() {
  ...
  const upload = (data) => {
    return new Promise((resolve, reject) => {
      ...
      const storageRefVar = storageRef(storage, `/img/About/${data.title}`);
  ...
  function firebaseWrite(inputData) {
    return new Promise((resolve, reject) => {
      ...
      set(databaseRef(db, `admin/About/${key}`), inputData)
        .then(() => {
          resolve();
        })
        .catch((error) => {
  ...
  function firebaseGet(key) {
    const dataRef = databaseRef(db, `admin/About/${key}`);
    ...
    onValue(usersRef, (snapshot) => {
      ...
      for (let obj in dataArray) {
        if (dataArray[obj].title === inputData.title) {
          ...
          set(databaseRef(db, `admin/About/${dataArray[obj].key}`), inputData);
          break;
        }
      }
